...
def CAN_periph_base_cleaner(periph: "Peripheral") :
	from sool.structure import MappingElement
...
def CRC_periph_cleaner(periph: "Peripheral") :
	from sool.structure import Register, Field, MappingElement
	PERIPH_VERSION_REGISTERS_cleaner(periph)
	if "POL" in periph :
		return
...
def DMA_periph_cleaner(periph: "Peripheral") :
	...
	if "CSELR" in periph :
		...
		for chip in reg.chips.chips :
			# For each registered chips
			# Detect if CSELR is not in the header.
	...
	pos = -1
	for chip in periph.chips.chips :
		if "CSELR" in chip.header_handler.periph_table["DMA"] :
			chips.add(chip)
			pos = chip.header_handler.periph_table["DMA"]["CSELR"].byte_offset
